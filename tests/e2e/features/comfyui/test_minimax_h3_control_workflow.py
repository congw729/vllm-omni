...
def test_shared_h3_defaults_and_portable_inputs(workflow):
    ...
    assert (values["width"], values["height"], values["fps"], values["num_frames"]) == (1344, 768, 24, 124)
    assert (values["num_frames"] - 5) % 17 == 0
    ...
    assert params["widgets_values"] == [3.0, 12.0, "16:9"]
    assert all(port["link"] is None for port in generate["inputs"] if port["name"] in {"frame", "references"})
...
def test_workflow_params_supply_the_backend_required_aspect_ratio(workflow, controlled):
    ...
    wire = _minimaxh3_params_builder(params, extra_params=extra)
